...
export class TextComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  ngOnChanges() {
    ...
    this.id = 'quill_' + this.elm._eid.str;
  }
  ...
  refsHaveChanged(refs1, refs2) {
    ...
    for (let i = 0; i < refs1.length; i++) {
      if (refs1[i].id !== refs2[i].id) return true;
    }
    return false;
  }
  updateRefs() {
    ...
    for (const it of output) {
      ...
      newrefs.pushUnique({ id: refelm._eid.str, elm: refelm });
    }
  ...
  async setEdit() {
    ...
    if (success) {
      this.html = this.elm.txts[this.pos];
  ...
  closeEdit($event) {
    ...
    this.html = this.html
      ...
      .replaceAll('<sup style="color: rgb(229, 115, 115);">®</sup>', '')
      .replaceAll(' = ', '&nbsp;=&nbsp;')
      .replaceAll(' < ', '&nbsp;<&nbsp;')
      .replaceAll(' > ', '&nbsp;>&nbsp;')
      .replaceAll(' &lt; ', '&nbsp;&lt;&nbsp;')
    ...
    if (this.elm.i18n[this.dS.locale].strs[this.pos] !== this.html) {
      this.elm.i18n[this.dS.locale].strs[this.pos] = this.html;
      // this.elm.txts.dirty = true;
      this.updateRefs();
      this.dS.rS.save(this.elm, this.pos, this.html, this.dS.locale);
    }
    //  else {
    //   this.elm.txts.dirty = false;
    // }
    // if (this.elm.txts.dirty) {
    //   this.dS.save(this.elm);
    // }
